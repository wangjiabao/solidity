...
contract JOB is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("JOB", "JOB") {

        uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        // Create a uniswap pair for this new token
        _tokenOwner = tokenOwner;
        botFeeAddress = address(0x8bB292B4973705d69dBFD54Bc5E717E9102587C9);
        ...
        // _isBlack[0xe865f2e5ff04B8b7952d1C0d9163A91F313b158f] = true;
        lpToken = usdt;
        RECV = new UsdtWrap(IERC20(address(this)));
        _mint(_tokenOwner, 5201314 * 10**18);
        divLpHolderAmount = 0;
        ...
        private
    {
        for(uint256 i = 0 ;i< lpAddresses.length;i++){
             if(lpPush[lpAddresses[i]] && (uniswapV2Pair.balanceOf(lpAddresses[i]) <= divLpHolderAmount)){
                _clrLpDividend(lpAddresses[i]);
             }else if(!Address.isContract(lpAddresses[i]) && !lpPush[lpAddresses[i]] && uniswapV2Pair.balanceOf(lpAddresses[i]) > divLpHolderAmount){
                _setLpDividend(lpAddresses[i]);
             }
        }
    }

    function setDivideAddress(address exa) public onlyOwner {
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) , "JOB: We can not withdraw self");
    ...
    function _transfer(
        ...
        if (takeFee) { 
            if(ammPairs[to] || ammPairs[from]){
                if(block.timestamp <= startTime.add(60)){
    ...
    function setUIniswapV2Pair(address newAddress) public onlyOwner {
        uniswapV2Pair = IUniswapV2Pair(newAddress);
    }
